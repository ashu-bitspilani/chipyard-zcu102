...
class ZCU106MIGIODDR(depth : BigInt) extends GenericParameterizedBundle(depth) {
  require((depth<=0x80000000L),"ZCU106MIGIODDR supports upto 2GB depth configuraton")
...
class zcu106mig(depth : BigInt)(implicit val p:Parameters) extends BlackBox
{
  ...
  val io = new ZCU106MIGIODDR(depth) with ZCU106MIGIOClocksReset {
